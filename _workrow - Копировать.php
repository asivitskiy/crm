...
<div align="right">Оплачено: <? echo $vnes_sum; ?></div>
  ...
  <td>	
	<table class="workrow" border="0" style="float: left; display:inline-block; width: 1195px; margin-top: 13px; ">
	  ...
	  <tr>
	  	...
	  	<td>
	  		<select  name="work_tech[]" tabindex="3" style="height: 35px" >
     								<option style="display: none"></option>
	  	...
	  	<td>
	  		<select  name="work_color[]" style="height: 35px" >
    								<option style="display: none"></option>
	  	...
	  	<td rowspan="2">
	  		...
	  		<table style="border: 1px dotted #D99798; border-radius: 3px; height:105px; padding-left: 4px; padding-right: 4px;" class="autoClear">
            		...
            		<tr>
            			...
            			<select style="width: 175px; height: 39px;" name="work_rashod_list[]">
            				<option></option>
            				<? if ($_GET['action'] == 'redact') {
							$current_work_tech = $main_table_data['work_tech'];
							$paylist_demand_sql = "SELECT `number` FROM `paylist_demands` WHERE `owner`='$current_work_tech'";
							$paylist_demand_query = mysql_query($paylist_demand_sql);
								while ($paylist_demand_data = mysql_fetch_array($paylist_demand_query)) {
								
								
							?>
            				<option <?if (($main_table_data['work_rashod_list'] == $paylist_demand_data['number']) and ($main_table_data['work_rashod_list'] <> '')) { echo 'selected';} ?>><? echo $paylist_demand_data['number']; ?></option>
            				<? }} ?>
            			</select>
...
<div>
	...
	<input type="submit" style="111" class="111" name="doubleflag" value="Дублировать заказ">
