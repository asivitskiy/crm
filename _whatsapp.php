...
function checkerror($json_responce,$blank_number,$message,$cur_phone_number) {
    ...
    //echo $blank_number."-> id ".$msg_number_array
    //[1]."<br>";
    $cur_date = date('YmdHi')*1;
    mysql_query("INSERT INTO `whatsapp_messages` (`error_w` , `order_number_w` , `text`, `message_id`, `date_w` , `current_t_number`,`json_responce`)
    VALUES ('$err_array[1]'  ,'$blank_number' , '$message',  '$msg_number_array[1]',' $cur_date','$cur_phone_number','$json_responce')");
    return $err_array[1];
}
...
$messages_sql ="SELECT *,SUM(works.work_count*works.work_price) as ordersumm FROM `order`
                ...
                GROUP BY order.order_number LIMIT 1";
...
while($messages_data = mysql_fetch_array($messages_array)) {
    ...
    if (checkerror($homepage,$ordernumber,$cur_msg_string,$cur_phone) == 0) {
...
while($messages_data = mysql_fetch_array($messages_array)) {
    ...
    $cur_msg_string = urlencode('Добрый день!').$br;
    $cur_msg_string .= urlencode('---------------------').$br;
    $cur_msg_string .= urlencode('ВАШ ЗАКАЗ ГОТОВ').$br;
    $cur_msg_string .= urlencode('---------------------').$br;
    $cur_msg_string .= urlencode('Это автоматическое сообщение о статусе заказа.').$br;
    $cur_msg_string .= urlencode('---------------------').$br;
    $cur_msg_string .= urlencode('Номер вашего заказа: '.$ordernumber).$br;
