<?
//$mess_text - сообщение с любыми символами  для отправки
//$phone_number - телефон через семерку для отправки
function send_whatsapp_wazzup($to,$message) {
$mess_text1 = urlencode($message);
$getstring = 'https://sivitskiy.pro/wazzup_test/?contact='.$to.'&messenger_type=whatsapp&text='.$mess_text1 ;
//$getstring_encoded = urlencode($getstring);
$ans = file_get_contents($getstring);
echo $ans;
sleep(2);
}
//////////////////////////////////////////////////////////////////////////////
...
while($messages_data = mysql_fetch_array($messages_array)) {
    ...
    $cur_msg_string = 'Добрый день!
'.$br;
        $mess_alert_for_mailer = ' (оформление)';
        if ($messages_data['notification_status'] == 2) {$cur_msg_string .= 'Данные заказа обновлены
            '.$br;$mess_alert_for_mailer = ' (обновление)';}
    $cur_msg_string .= 'Номер вашего заказа '.$ordernumber.$br;
    $cur_msg_string .= '
Сумма к оплате: '.number_format($messages_data['ordersumm'], 2, ',', ' ').' руб.
'.$br;
    $cur_msg_string .= 'Ваш менеджер '.$manager.$br.'
';
    $cur_msg_string .= 'Типография АДМИКС
'.$br;
    $cur_msg_string .= 'Гурьевская, 78 (левый цоколь)
'.$br;
    $cur_msg_string .= 'тел. +7(383)207-56-42
'.$br.'сот. +7(923)240-10-20
'.$br;
...
';
    ...
    send_whatsapp_wazzup($cur_phone,$cur_msg_string);
...
while($messages_data = mysql_fetch_array($messages_array)) {
    ...
    $cur_msg_string = 'Добрый день!
'.$br;
    $cur_msg_string .= 'Ваш заказ готов.
'.$br;
    $cur_msg_string .= 'Номер вашего заказа '.$ordernumber.$br;
    $cur_msg_string .= '
Сумма к оплате: '.number_format($messages_data['ordersumm'], 2, ',', ' ').' руб.
