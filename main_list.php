...
<?	// все заказы (и свои и чужие, НЕГОТОВЫЕ)
	...
	if (($_GET['filter'] == "client")) {
		...
		$query = "SELECT * FROM `order` WHERE ((`contragent` = '$argument')) ORDER BY `date_in` DESC ";
			...
			$client_card_query = mysql_query($client_card_sql);
			$client_card_data = mysql_fetch_array($client_card_query); ?>
	...
	if (($_GET['filter'] == "demand") and (isset($_GET['list']))) {
	...
	if (($_GET['filter'] == "contragent_paydemand") and (isset($_GET['argument']))) {
	...
	{		
		...
		$reorder_flag		= 0;
			...
			$res_works_1 = mysql_query($query_works_1);
				while($row_works_1 = mysql_fetch_array($res_works_1))
					{ 	if ($row_works_1['gotov'] <> 0) {$ready_counter = $ready_counter + 1;}
						...
						$amount_order = $amount_order+(($row_works_1['work_price']*1)*($row_works_1['work_count']*1)); 
					}
			...
			$err_empty_worktech_array = mysql_query($err_empty_worktech_sql);
				while($err_empty_worktech_data = mysql_fetch_array($err_empty_worktech_array))
	...
	if (dig_to_d($row['date_in']) <> dig_to_d($prev)) {$prev = $row['date_in'];	?>
  	...
  	<? } ?>
		...
		<? //расчет долга по заказу и изменение его цвета в зависимости от статуса оплаты
				...
				$order_money_income_array = mysql_query("SELECT * FROM `money` WHERE `parent_order_number` = '$order_number'");
				while ($order_money_income_data = mysql_fetch_array($order_money_income_array)) {
