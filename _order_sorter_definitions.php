...
$client_list_sql = "SELECT * FROM `contragents` ORDER by `relativity` DESC";
...
<table style="margin-top: -4px; margin-bottom: -8px; font-family:'Gill Sans', 'Gill Sans MT', 'Myriad Pro', 'DejaVu Sans Condensed', Helvetica, Arial, 'sans-serif'; border-color: white; border-radius: 3px; width: 850px;">
	<tr>
		...
		<td rowspan="2" align="right"><input type="button" button onclick="location.href = '?action=showlist&filter=client&argument=<? echo $client_list_data['id']; ?>'" value="Карточка клиента">&nbsp;<input type="button" onclick="location.href = '?action=client_list&red_num=<? echo $client_list_data['id']; ?>'" value="Удалить"></td>
	</tr>
	<tr height="15">
