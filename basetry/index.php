...
<? 	$hrefs[0][0] = "?action=new";
	...
	$message_array = mysqli_query($connect,$message_query);
	$a = mysqli_num_rows($message_array);
	...
	$newmessage_flag_array = mysqli_query($connect,$newmessage_flag_sql);
	
	if (mysqli_num_rows($newmessage_flag_array) == 0) {
...
$res_4 = mysqli_query($connect,$query_4);
...
<div class="center_block">
	...
	<?
	$dolg_opl_data = mysqli_fetch_array(mysqli_query($connect,"SELECT SUM(money.summ) `opl` FROM `money`"));
	$dolg_neopl_data = mysqli_fetch_array(mysqli_query($connect,"SELECT SUM(works.work_price * works.work_count) `neopl` FROM `works`"));
