...
<div class="cashbox_wrapper">
	...
	<?
		...
		if ($_SESSION['supervisor'] == 1) {$paylist_demands_sql = "SELECT * FROM `paylist_demands` ORDER by `checked` ASC , `closed` ASC, `owner` DESC";}
		...
		//дальше идёт выемка из работ чтобы взаимозачет выполнить
			...
			?>">
				<td style="width: 200px;">
					<a href="?action=showlist&filter=demand&owner=<? echo $owner_id; ?>">
					<? echo $paylist_demands_data['owner']; ?>
					</a>
				 </td>
				
				<td style="width: 200px;" align="right">
					<a href="?action=showlist&filter=demand&list=<? echo $paylist_demands_data['number']; ?>">
